...
def handle_notification(application, ieee, header, counter, command_id, payload, payload_length, mic):
    ...
    if expected_mic is not None and expected_mic != mic:
        logging.debug("[zgp.handle_notification] Wrong mic : %s, expected mic %s, ignore frame", hex(mic),
                      hex(expected_mic))
        return
    ...
    try:
        ...
        payload = ()
        pass
    logging.debug("[zgp.handle_notification] Green power frame ieee : %s, command_id : %s, payload : %s,counter : %s",
