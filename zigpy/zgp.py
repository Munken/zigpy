        ...
        try:
            ...
            if type == 'bellows':
                ...
                if command_id == 0xe0:
                    ...
                    create_device(self.app, addr)
                else:
                    LOGGER.debug("handle_notification")
                    handle_notification(self.app, addr, header, counter, command_id,
...
def create_device(application, ieee, type=None, remoteCommissioning=False):
    ...
    if not remoteCommissioning and (0x9997 not in gateway.endpoints[endpoint_id].out_clusters[cluster_id]._attr_cache or
                                    gateway.endpoints[endpoint_id].out_clusters[cluster_id]._attr_cache[
                                        0x9997] < time.time()):
        LOGGER.debug("[zgp.create_device] Not in permit joining mode and not in remoteCommissioning")
        return None
    LOGGER.debug("[zgp.create_device] Device %s not found, create it", ieee)
    nwk = 32766
    for i in range(32766, 65535):
        nwk = i
        try:
            application.get_device(nwk=i)
        except KeyError:
            break
    ...
    if type is not None and type in devices:
        ...
        for id in in_clusters:
            LOGGER.debug("[zgp.create_device] Add input cluster id %s on device %s", id, ieee)
            ep.add_input_cluster(id)
        for id in out_clusters:
            LOGGER.debug("[zgp.create_device] Add output cluster id %s on device %s", id, ieee)
...
def handle_notification(application, ieee, header, counter, command_id, payload, payload_length, mic):
    ...
    if command_id not in commands:
        ...
        return
    expected_mic = calcul_mic(application, ieee, header, counter,
                              command_id.to_bytes(1, "little") + payload.to_bytes(payload_length, "little"),
                              payload_length + 1)
    if expected_mic is not None:
        LOGGER.debug("[zgp.handle_notification] Mic : %s, expected mic %s : OK", hex(mic), hex(expected_mic))
    if expected_mic is not None and expected_mic != mic:
        LOGGER.debug("[zgp.handle_notification] Wrong mic : %s, expected mic %s, ignore frame", hex(mic),
                      hex(expected_mic))
        return
    ...
    try:
        ...
        payload = ()
        pass
    LOGGER.debug("[zgp.handle_notification] Green power frame ieee : %s, command_id : 0x%02x, payload : %s,counter : %s",
                  ieee, command_id, payload, counter)
    ...
    if counter is not None:
        ...
        if 0x9999 in attributes and attributes[0x9999] >= counter:
            LOGGER.debug("[zgp.handle_notification] Already get this frame counter,I ignoring it")
            return
    ...
    if clusterid is not None and type == "CLUSTER_COMMAND":
        if clusterid not in dev.endpoints[1].out_clusters:
            ...
            application.device_initialized(dev)
        dev.endpoints[1].out_clusters[clusterid].handle_message(
            foundation.ZCLHeader.cluster(application.get_sequence(), zcl_command_id), value)


def handle_message(application, data):
    data = data[0]
...
def calcul_mic(application, ieee, header, counter, payload, payload_length):
    if ieee not in application.devices:
        return None
    dev = application.devices[ieee]
    ...
    LOGGER.debug(
        "[zgp.calcul_mic] Calcul mic of green power frame for %s on header : 0x%s, src_id : 0x%s, counter : 0x%s, payload : 0x%s, payload length : %s",
        ieee, header.hex(), src_id.hex(), counter.hex(), payload.hex(), payload_length, )
...
def setKey(device, key):
    if key is None:
        LOGGER.debug("[zgp.setKey] Remove key for device " + str(device.ieee))
        del device.endpoints[endpoint_id].in_clusters[cluster_id]._attr_cache[0x9998]
        return
    if not isinstance(key, (bytes)):
        key = key.to_bytes(16, "big")
    LOGGER.debug("[zgp.setKey] Set key for device " + str(device.ieee) + ' to ' + str(key))
